...
export class PoprawaPoMiloszuCoPozmienialNazwyWEncjach1768816708673 implements MigrationInterface {
    name = 'PoprawaPoMiloszuCoPozmienialNazwyWEncjach1768816708673'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`seans\` CHANGE \`okladka_url\` \`okladka_url\` varchar(255) NULL`);
        ...
        await queryRunner.query(`ALTER TABLE \`seans\` ADD CONSTRAINT \`FK_ae46c17c73862e7cdf64b73e320\` FOREIGN KEY (\`sala_id\`) REFERENCES \`sala\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`rezerwacja\` ADD CONSTRAINT \`FK_d52b9749a1821ceafdf3720115f\` FOREIGN KEY (\`sala_id\`) REFERENCES \`sala\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`rezerwacja\` ADD CONSTRAINT \`FK_36c81b2eb19acf44209957c0db4\` FOREIGN KEY (\`siedzenie_id\`) REFERENCES \`siedzenie\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`rezerwacja\` ADD CONSTRAINT \`FK_a845aa3f0d53035c80ffdabefed\` FOREIGN KEY (\`seans_id\`) REFERENCES \`seans\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`rezerwacja\` ADD CONSTRAINT \`FK_cdd58116a197522fd6e8f7fdcb7\` FOREIGN KEY (\`uzytkownik_id\`) REFERENCES \`uzytkownik\`(\`uzytkownik_id\`) ON DELETE SET NULL ON UPDATE NO ACTION`);
    }
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`seans\` CHANGE \`okladka_url\` \`okladka_url\` varchar(255) NULL DEFAULT 'NULL'`);
